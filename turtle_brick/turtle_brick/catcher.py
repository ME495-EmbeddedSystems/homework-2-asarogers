...
class Catcher(Node):
    ...
    def __init__(self):
        ...
        self.stopGravityClient = self.create_client(
            StopGravity, "/stop_gravity")
    ...
    def canCatch(self):
        ...
        distanceFromBrick = math.sqrt(
            (brickX - turtleX) ** 2 + (brickY - turtleY) ** 2)

        timeUntilMaxVelocity = Vmax / a

        distanceCoveredDuringAcceleration = (
            1 / 2) * a * timeUntilMaxVelocity**2
        timeUntilTurtleReachesBrick = 0

        if distanceCoveredDuringAcceleration >= distanceFromBrick:
            timeUntilTurtleReachesBrick = math.sqrt(
                (2 * distanceFromBrick) / a)
        else:
    ...
    def sendGoalPose(self, x, y, z):
        ...
        request.goal_pose.pose.orientation = Quaternion(
